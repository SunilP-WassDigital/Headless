        ...
        <div className="prose mt-8 border-t pt-8" key={post?.sys.id}>
          ...
          <div className="mb-8 flex">
            <small className="mb-0 text-base text-gray-500 pt-5">
              {"Published On: "}{format.dateTime(new Date(post.publishedDate))}
            </small>
            {Boolean(post.author) && (
              <small className="text-base text-gray-500 pt-5">, by {post.author.name}</small>
            )}
            {Boolean(post.author?.avatar?.url) && (
              <Image
          ...
          <Image
            alt={post?.featuredImage?.description ?? 'Featured Image'}
            height={500}
          ...
          <nav className="flex sm:justify-center space-x-4">
            {post.relatedBlogPostsCollection?.items.map((relatedPage: any) => (
              <Link
                ...
                href={`/contentful/${relatedPage.slug}`}
                className="delay-100 duration-100 transform hover:scale-105 transition ease-linear bg-gray-100 px-2 py-2 m-4 inline text-blue-900"
              >
                {relatedPage.title}
              </Link>
            ))}
